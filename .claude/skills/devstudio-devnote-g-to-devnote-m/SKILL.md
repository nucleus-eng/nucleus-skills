...
- **Strip all `@claude` comments** — authors use `@claude` inline in the Doc to give
  instructions to the TA (e.g. `@claude these tables should be a tabset`). Act on each
  instruction, then remove the comment text entirely from the MyST output. Never carry
  `@claude` text forward into `main.md` — it will render as body text and break the
  document. Pay particular attention to `@claude` comments that appear between the
  YAML frontmatter (`---`) and the abstract block (`+++`), where stray text is
...
**Always** emit this block in `main.md` immediately after the frontmatter (before `# Overview`) —
it is mandatory even when no abstract text exists.

Read the Overview section and use the first 2–3 sentences as an abstract draft if they
form a coherent standalone summary. Wrap with a REVIEW flag asking the author to confirm:

```markdown
+++ { "part": "abstract" }
<!-- REVIEW: Abstract drafted from Overview — confirm or revise before publishing. -->
[first 2–3 sentences from Overview]
+++
```

If the Overview is too long or procedural to yield a clean 2–3-sentence draft, emit the
block empty with a REVIEW comment only (do not fabricate or force a summary):
...
The `title:` field in `main.md` frontmatter and `curvenote.yml` must be **identical** —
this is the primary title consistency check. There is no standalone H1 title heading;
`# Overview` is the first heading in the body.

**Do NOT add a `parts: abstract:` entry to `curvenote.yml`.** The `+++` delimiter block
in `main.md` is sufficient — curvenote reads it natively. Adding `parts: abstract: file:
main.md` to `curvenote.yml` triggers an `abstract.0 must be string` validation error
because the schema expects a string value, not an object. Leave a comment only:
```yaml
  # parts: abstract — handled by +++ { "part": "abstract" } block in main.md
```
...
**Acknowledgements**: always add a standard `# Acknowledgements` section immediately
before `# Conclusions and next steps`. Use this boilerplate (verbatim — sourced from
the Schmidt Sciences DevCell project description at syncellwiki.org):
...
block rather than replacing it. The funding entry in `curvenote.yml` (see Step 5) and
this section are complementary — both are required.

**Data availability**: always add a `+++ { "part": "data_availability" }` block at the
very end of `main.md` (after Conclusions). When data files are included in the devnote
bundle, use:

```markdown
+++ { "part": "data_availability" }
All data supporting this DevNote are included in the accompanying data bundle,
available for download above.
+++
```

If data will not be bundled, flag with REVIEW and leave the statement blank.
...
:::
```
The seqviz plugin accepts either a **local path** or a **GitHub blob URL** — the
latter is auto-rewritten to `raw.githubusercontent.com` at build time
(`nucleus-eng/devnote-template#5`, merged). Prefer the GitHub URL when the construct
already lives in `nucleus-eng/DNA`:
...
Only copy the `.gb` file locally to `plasmids/` when it is not yet in `nucleus-eng/DNA`
(new/unregistered construct). Note: the GitHub URL approach adds a build-time network
dependency — builds need internet access and will break if the file is moved or the
repo goes private. Do not use `:viewer:` — not a recognized option, emits a warning.
...
**Composition table placement**: reaction composition tables always go in the `# Methods`
section (under the relevant experiment subsection), not in `# Results`. In the Results
narrative, replace any "as shown in Table N" prose with a `{ref}` MyST cross-reference
to the Methods table — MyST renders these as hover cards, so the reader can inspect the
composition without leaving the Results:
...
Reactions were prepared as described in {ref}`tbl-exp1-cytosol` and {ref}`tbl-exp1-pure`.
```

This avoids duplicating table content across sections while keeping Results readable.
...
**Citations**: when the DevNote(G) contains a DOI URL (e.g.
`https://doi.org/10.1038/90802`) without an explicit author/year string alongside it,
emit the DOI as a plain hyperlink and add a REVIEW flag:
```markdown
([doi:10.1038/90802](https://doi.org/10.1038/90802)) <!-- REVIEW: add author/year citation string -->
```
Never infer or fabricate the author name or publication year from the DOI — the DOI
resolver knows who wrote the paper; the MyST author does not. A fabricated citation
("Smith et al. 2019") that is wrong is worse than a bare DOI link.
...
project:
  ...
  authors:
    - name: <name>
      ...
      roles:                   # CRediT taxonomy — include all that apply
        - Conceptualization
        - Data curation
        - Investigation
        - Methodology
  ...
  jupyter: true
  funding:
    - statement: This work was supported by the Schmidt Sciences Developer Cell (DevCell) project.
      sources:
        - name: Schmidt Sciences Foundation
  exports:
    - format: meca
      id: data-bundle
    - format: typst
